...
test_that("plot_edge_diff_forest tile works", {
  ...
  boot <- boot_glasso(net, iter = 100, cs_iter = 25, seed = 1, centrality = c("strength", "expected_influence"))
...
test_that("plot_edge_diff_forest chord works", {
  ...
  net  <- build_network(srl_strategies, method = "glasso")
  boot <- boot_glasso(net, iter = 100, cs_iter = 25, seed = 1, centrality = c("strength", "expected_influence"))
  # Named node colours used to leak into data.frame() row names ("short variable").
  expect_no_warning(
