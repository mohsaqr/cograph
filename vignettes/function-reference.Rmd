...
The `centrality()` function computes all measures at once.

```r
centrality(net)
#   node degree strength betweenness closeness eigenvector pagerank
# 1    A      2      1.3         0.0       1.0   0.7071068    0.333
# 2    B      2      0.8         0.5       1.0   0.7071068    0.333
# 3    C      2      1.1         0.5       1.0   0.7071068    0.333
```

Individual centrality functions are also available:

| Function | Description |
|----------|-------------|
| `centrality_degree()` | Degree centrality |
| `centrality_strength()` | Weighted degree (strength) |
...
| `centrality_hub()` | Hub scores (HITS) |
| `centrality_authority()` | Authority scores (HITS) |
| `centrality_constraint()` | Burt's constraint |
| `centrality_coreness()` | K-core decomposition |
...
| `filter_nodes()` | Filter nodes by centrality expression |
| `network_summary()` | Comprehensive network statistics |
...
hubs <- filter_nodes(net, degree >= 3)

# Keep nodes with high PageRank
important <- filter_nodes(net, pagerank > 0.1)
...
| **Analysis** | `centrality()`, `detect_communities()`, `network_summary()` |
| **Filtering** | `filter_edges()`, `filter_nodes()` |
