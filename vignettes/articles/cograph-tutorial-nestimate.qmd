...
- **Multi-cluster multi-level analysis** (`csum`, `summarize_clusters`) --- aggregates node-level networks into cluster-level summaries for hierarchical visualization.
...
`summarize_clusters()` provides the same result through a slightly different interface. When given a `netobject`, it uses the pre-computed weight matrix:
...
When no strong theoretical grouping exists, `Nestimate::cluster_network()` discovers clusters algorithmically. It clusters the raw sequence data (using PAM by default) and builds a separate network per cluster, returning a `netobject_group`:

```{r}
#| label: mcml-datadriven
#grp_clustered <- Nestimate::cluster_network(group_regulation_long, k = 3)
#names(grp_clustered)
```
...
| Data-driven | `Nestimate::cluster_network(data, k)` | raw data + k | `netobject_group` | Exploratory clustering |
...
# Supported Dispatch Chain

Several Nestimate object types route through `splot()` automatically. The following table summarizes the supported classes covered in this tutorial:
...
| `mcml` | `csum()` / `summarize_clusters()` | Two-layer MCML plot |
