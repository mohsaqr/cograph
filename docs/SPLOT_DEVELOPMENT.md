...
3.  **Simplicity**: Direct coordinate system without NPC unit
    conversions
4.  **xspline()**: Produces smoother, more natural curves than grid’s
    bezier curves

### Design Decisions

1.  **Curve Direction for Reciprocal Edges**: When two nodes have edges
    in both directions (A→B and B→A), they curve in opposite directions
    forming an “ellipse” shape - one curves inward, one curves outward.
    This prevents overlap and matches qgraph behavior.

2.  **Default Behavior** (`curves=TRUE`): Only reciprocal edges are
    curved; single edges remain straight. This is the most common use
    case for network visualization.

3.  **Curve Modes**:

    - `curves=FALSE`: All edges straight
    - `curves=TRUE` or `"mutual"`: Only reciprocal edges curved
      (default)
    - `curves="force"`: All edges curved

4.  **Inward Curve Logic**: For single edges with `curves="force"`, the
    curve should bend toward the network center. This is determined by:

    - Calculate network center (mean of all node positions)
    - For each edge, use cross product to determine which side of the
      edge the center is on
...
- All node shapes (circle, square, triangle, diamond, pentagon, hexagon,
  star, heart, ellipse, cross)
- Pie chart nodes
- Donut chart nodes
...
**Problem**: When `curves="force"` is used, single (non-reciprocal)
edges should curve inward toward the network center. Previously, the
...
center is to the left, negative (bend right) if center is to the right
4. Negative curves (reciprocal edges curving outward) keep their
original sign

### 2. ~~HIGH PRIORITY: Resolution/DPI~~ DONE

**Problem**: Output resolution needs to be higher for publication
quality.

**Solution**: Added `res` parameter to control DPI for raster outputs
(PNG, JPEG, TIFF). Default is now 600 DPI for publication-quality
output. The parameter is passed to
[`grDevices::png()`](https://rdrr.io/r/grDevices/png.html),
[`grDevices::jpeg()`](https://rdrr.io/r/grDevices/png.html), and
[`grDevices::tiff()`](https://rdrr.io/r/grDevices/png.html) functions.

### 3. ~~MEDIUM PRIORITY: Edge Label Positioning~~ DONE

**Problem**: Edge labels on curved edges were positioned directly on the
...
0.03 user coordinates provides good separation

### 4. ~~MEDIUM PRIORITY: Legend Support~~ DONE

**Problem**: Legend only supported node groups, missing edge colors and
node sizes.
...
parameter, default TRUE) 3. **Node sizes**: Circles showing
small/medium/large scale (controlled by `legend.node.sizes` parameter,
default FALSE)
...
**Problem**: Users need to visualize uncertainty in edge weights (e.g.,
confidence intervals from statistical models).

**Solution**: Added CI underlay feature that draws each edge twice: 1.
**Underlay (first pass)**: thicker, dashed, semi-transparent “halo”
representing uncertainty 2. **Main edge (second pass)**: normal
rendering on top
...
character \| NA \| Underlay color (NA = use main edge color) \| \|
`edge_ci_style` \| numeric \| 2 \| Line type: 1=solid, 2=dashed,
...
**Problem**: Edge labels needed to display statistical information like
estimates, confidence intervals, p-values, and significance stars in
customizable formats.

**Solution**: Added a template-based label system with preset styles and
custom templates.

**Style Presets:** \| Style \| Template \| Example Output \|
\|——-\|———-\|—————-\| \| `"none"` \| (no labels) \| \| \| `"estimate"`
\| `"{est}"` \| `0.45` \| \| `"full"` \| `"{est} {range}"` \|
`0.45 [0.32, 0.58]` \| \| `"range"` \| `"{range}"` \| `[0.32, 0.58]` \|
\| `"stars"` \| `"{stars}"` \| `**` \|

**Template Placeholders:** \| Placeholder \| Source \| Example \|
\|————-\|——–\|———\| \| `{est}` \| edge weight \| `0.45` \| \| `{range}`
\| ci_lower + ci_upper \| `[0.32, 0.58]` \| \| `{low}` \| ci_lower \|
`0.32` \| \| `{up}` \| ci_upper \| `0.58` \| \| `{p}` \| p-value \|
`p=0.012` \| \| [stars](https://r-spatial.github.io/stars/) \|
significance stars \| `**` \|
...
\| `edge_label_stars` \| \* \| NULL \| Stars: character, logical, or
numeric p-values \|
...
For very large networks (\>500 nodes), consider: - Batch drawing of
similar elements - Reducing xspline resolution for distant edges -
Level-of-detail rendering

## Code References

- Main curve rendering: `R/splot-edges.R:draw_curved_edge_base()` (line
  ~86)
- Inward direction logic: `R/splot-edges.R:render_edges_base()` (line
  ~380)
...
- `R/render-edges.R` - Grid edge rendering (has aspect ratio fixes that
  may be useful)
