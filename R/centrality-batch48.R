...
#' node's \emph{neighbor information} from degree and its \emph{topological
...
#' neighborhood \eqn{\tau(v)=N(v)}. The triangle share is
...
#' collapses to the neighbors and \eqn{C(v)} becomes
...
#'
#' \strong{Both neighborhoods are open, and a node contributes to its own
...
#' \eqn{\tau(v)} is the open neighborhood. It follows -- the paper does
