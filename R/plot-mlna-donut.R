# =============================================================================
# EXPERIMENTAL / SANDBOX — plot_mlna() 3D multilayer with DONUT nodes + themed
# styling. SEPARATE from R/mlna.R; does NOT touch plot_mlna(). Full working copy
# renamed plot_mlna_donut(), with ONLY the node-drawing block swapped from
# graphics::points() pch symbols to splot's draw_donut_node_base(). Each node's
# donut fill = its self-transition proportion (diag(weights)/rowSums), the same
# semantics used in the mcml donut trial. Marked @noRd: no export, no man page.
# Call via cograph:::plot_mlna_donut() after load_all().
#
# New args vs plot_mlna():
#   node_donut_inner_ratio   donut hole size (0-1)
#   node_border_width        donut ring border width (thin)
# =============================================================================

#' @noRd
plot_mlna_donut <- function(
    model,
    layer_list = NULL,
    community = NULL,
    layout = "horizontal",
 ...
 skew_rad <- skew_angle * pi / 180
  ...
  for (i in rev(seq_len(n_layers))) {
    ...
    # --- Draw nodes as DONUTS (fill = node self-transition proportion) ---
    donut_r <- node_size * 0.10 / size_scale
    for (nd in idx) {
      tot <- sum(weights[nd, ], na.rm = TRUE)
      self_prop <- if (is.finite(tot) && tot > 0) weights[nd, nd] / tot else 0
      self_prop <- max(0, min(1, self_prop))
      draw_donut_node_base(
        x = x_pos[nd], y = y_pos[nd], size = donut_r,
        values = self_prop,
        colors = layer_colors[i],
        inner_ratio = node_donut_inner_ratio,
        bg_color = grDevices::adjustcolor(layer_colors[i], 0.35),
        center_color = "white",
        border.col = "gray20",
        border.width = node_border_width * edge_scale,
        show_value = FALSE
      )
    }
