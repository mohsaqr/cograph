...
**Date:** 2026-02-10 **Branch:** `cograph_merged` **Base:**
`cograph_plotonly` (commit `fa91917`) **Upstream:** `sonsoleslp/cograph`
(main)
...
This merge combines the plotting-focused `cograph_plotonly` branch with
selected infrastructure and analysis functions from upstream, creating a
comprehensive network visualization package with basic analysis
capabilities.
...
Created a dedicated folder for estimation/validation functions that
require raw data. These are kept for future integration but excluded
...
**Rationale:** These functions require raw sequence data and the `tna`
package for estimation. They are preserved for future use when
estimation capabilities are added back.
...
**Benefits:** - Automated testing on Linux, macOS, Windows - Automatic
package website generation - Code coverage tracking with codecov
...
With `detailed = TRUE`, also computes mean/sd/median for degree,
strength, betweenness, closeness, eigenvector, pagerank, constraint, and
local transitivity.
...
**Test Coverage:** - Basic matrix input - Specific measures selection -
Individual centrality functions - Normalization - Directed networks -
cograph_network input - igraph input - Sorting and rounding - Error
handling - network_summary basic and detailed modes -
...
**R CMD check:** - Errors: 0 - Warnings: 0 - Notes: 4 (minor,
non-blocking)
...
These require raw sequence data and are kept aside for future
integration:

| Function | Purpose |
|----|----|
| [`bootstrap()`](http://sonsoles.me/tna/reference/bootstrap.md) | Edge significance via resampling |
| [`permutation_test()`](http://sonsoles.me/tna/reference/permutation_test.md) | Network comparison p-values |
| [`estimate_cs()`](http://sonsoles.me/tna/reference/estimate_centrality_stability.md) | Centrality stability coefficient |
| `disparity_filter()` | Backbone extraction |

### Upstream R6 Structure

The upstream uses an R6 `CographNetwork` class. We kept our lean S3
named list structure which is simpler and more compatible with the
existing codebase.

### Upstream Layout Functions

New layout functions in upstream (`layout_grid_fn`, `layout_random_fn`,
`layout_star_fn`, `layout_bipartite_fn`) use the R6 class and were not
imported.
...
2.  **Push to remote** when satisfied:
    `git push -u origin cograph_merged`
3.  **Create PR** to merge into main branch
4.  **Update documentation** if needed for new functions
5.  **Consider** adding estimation functions back when raw data support
    is implemented
...
**Total lines added:** ~3,200 **Total lines removed:** ~120
