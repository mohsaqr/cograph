...
Every wrangling verb takes any supported input, takes its options as named
arguments, and returns a network. `as.data.frame()` gives the tidy edge table,
with endpoints as labels; `as.data.frame(what = "nodes")` gives the nodes. See
`?network_wrangling` for the whole family.
...
Centrality measures are computed lazily, and any measure `centrality()` knows
...
Filtering edges does not remove nodes, matching igraph and tidygraph; call
`remove_isolates()` when you want them gone.
...
`centrality()` computes node centrality measures and returns one tidy data frame.
The built-in `student_interactions` edge list works directly: no igraph object,
no matrix conversion, no setup code.
...
That one call returns the default centrality table: degree, strength, closeness,
betweenness, eigenvector centrality, and PageRank. Individual functions return
named vectors when you want one measure only:
